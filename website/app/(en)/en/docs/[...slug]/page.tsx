...
export function generateStaticParams() { return pages.filter(p => p.lang === 'en' && p.slug).map(p => ({ slug: p.slug.split('/') })); }
import { DocsPage } from '../../../../../components/docs-page';
export default async function Page({ params }: { params: Promise<{ slug: string[] }> }) {
 const { slug } = await params;
 return <DocsPage en={true} slug={slug.join('/')} />;
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string[] }> }) {
 const { slug } = await params; const page = pages.find(p => p.lang === 'en' && p.slug === slug.join('/'));
