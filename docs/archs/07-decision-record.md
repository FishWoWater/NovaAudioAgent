...
| Native tool surface | Five stable host/native tools; external MCP is user-selected context cost, with candidate realtime budget B=24 pending live validation and no silent truncation | Growing a mandatory native tool list or silently trimming user MCP tools |
| Vision boundary | AgentController `vision` owns hidden `watch`/`guard`; foreground VLMs receive turn-scoped images and monitors own capture and inference | Exposing monitor channels as dispatch executors or treating camera data as instructions |
