...
> 日期：2026-09-10，2026-09-12 补记忆底座与 IM · 当时规划状态：**全部里程碑未开始**；这不是当前实现进度声明。
...
| **记忆底座** | 06 卷 `evidence_record` / `entry_revision` schema 与 fixtures；merge 纯函数；fold 与四种投影；03 卷要的 `list` / `get` / `correct` / `forgetEntry` 由此提供 | 06 卷评审通过；契约钉住 | 06 卷场景 12、13、14、16、17 通过；VoiceMem 迁移路径拍板 | ⬜ 未开始 |
...
8. 记忆收敛为一套三阶段底座：账本只追加且存原文，条目是修订日志，视图只读。VoiceMem 可迁为写入方；
   已退役的 Workspace Graph 不迁入。用户纠正和模型合并是同一种修订记录。
...
| 记忆 | personal-memory 端口（recall / remember / 按来源 forget）、开发者 memory-board | 没有统一的证据账本与修订日志；没有逐条 ID、版本、列表、按条目纠正 / 忘记；没有用户视角的记忆条目和纠正 UI |
