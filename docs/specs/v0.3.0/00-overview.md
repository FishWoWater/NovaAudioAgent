...
- **M6 两项都依赖契约对象先在 02、03 卷钉住并有 fixtures**，以及"记忆底座"里程碑（06 卷 `evidence_record` / `entry_revision` schema 与 fixtures，Workspace Graph 作为第一写入方迁入）与 02 卷的持久化交付 / 忽略台账先落地。03 卷 §2.1 的 `list` / `get` / `correct` / `forgetEntry` 由底座提供。契约字段在 02（`feed_item`）、03（`memory_entry`）、06（底座）给出完整表；本卷只给概念定义。
