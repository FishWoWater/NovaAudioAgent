...
1. 评审时存在三套记忆（会话黑板、VoiceMem 个人记忆、旧图谱），03 卷要求的
   逐条 ID / 版本 / origin / 纠正传播在任何一套里都不完整。旧图谱已于 2026-09-13 退役；
   新底座直接围绕证据与修订定义，不恢复旧图谱。
2. 记忆层内部应按**数据变更方式**分阶段，而不是按参考项目或按"人 / 工作"分库。变更方式只有三种：
   只追加、受控修订、只读重算。
3. "有迹可循"是每条修订都必须带的 `evidence_refs`，不是独立的一层；把它单独立层会让两层互相持有指针，
   永远解不开。

## 1. 现有基础（2026-09-13 整合核实）

- `runtime/src/core/memory.ts`：会话内运行时黑板（`Memory` / `Channel`，按 handoff 通道编号的 `MemoryItem`），
  `docs/archs/02-memory.md` 的 L0。**不进本卷底座**；它是当前会话事实，不是持续理解。
- 旧图谱的 schema、SQLite store、worker、projector 和迁移工具已退役。新底座从
  `evidence_record` / `entry_revision` 起步，不复用旧图谱实现；既有图谱 SQLite 用户数据不自动删除。
  原敏感内容与路径策略保留于 `runtime/src/memory/sensitivity.ts`，知识库与桌面策略继续复用。
...
- `runtime/src/core/context-view.ts`：唯一面向模型的同步有界投影。
- `runtime/src/composition/realtime-assembly.ts` 的 `responseAdaptationFor()` 把 `<reply_preferences>` 同步注入；
  `runtime/src/core/tool-schema.ts` 的 `memory__recall` 工具带 `source: session | personal`。
  实时语音路径没有逐轮 prompt 组装，记忆只能经 host item 或工具结果进入模型。
- `runtime/src/core/suggestions.ts` + `runtime/src/realtime/floor.ts`：Suggestion Pool 与说话权仲裁，是本卷视图层的消费者。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `id` | string | 稳定；`entry_revision.evidence_refs` 的引用目标；形态由本卷契约定义 |
...
| `trust` | 沿用 `runtime/src/core/events.ts` 的 `trustSchema` | 外部内容一律低信任；`user_correction` 最高 |
...
| `entity` | 共享 | `entity_kind: person \| project \| workspace`、别名列表 | 实体归一按新底座定义；person 归一待评审 |
| `topic` | Work-centric | 标签 + 出现范围 | 授权目录（today.ai 式关键词，证据指向文件） |
| 工作区与关系条目（具体 kind 待定义） | Work-centric | 按 evidence / entry 契约定义实体与关联 | 来源候选经 merge 写修订，不恢复旧图谱 schema / projector |
...
- 抽取结果全部是 `origin = inferred`，除非来源本身是用户在对话中的明确表达（`user_confirmed`，
  非 ASR 原始转写，区分 `user_transcript` 与 `user_confirmed`）。
...
- 不在 06 卷底座之外新建记忆存储；VoiceMem 可改为写入方；已退役的旧图谱不作为迁移实现基础。
...
| person 实体归一 | 采用 candidate / confirmed / suppressed 状态机制 / 只按连接器给的稳定 ID，不做跨来源归一 | 前者能把飞书里的人和邮件里的人对上，误合并风险需 ASR 式置信上限 |
| 历史图谱用户数据 | 新底座是否导入、如何验证与回滚需另行定义 | 不自动删除历史 SQLite，不复用已退役代码 |
