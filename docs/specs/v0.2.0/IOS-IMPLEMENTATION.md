...
**新增：** `runtime/src/server-entry.ts`、`runtime/src/server/server-config.ts`、`runtime/src/composition/production-composition.ts`、`runtime/test/server-entry.test.ts`、`docs/deployment/remote-server.md`。
...
**修改：** `runtime/src/desktop/desktop-session.ts`、`desktop-session.ts`，必要时 `desktop-session.ts`、`realtime/service.ts`、`realtime-assembly.ts`。
