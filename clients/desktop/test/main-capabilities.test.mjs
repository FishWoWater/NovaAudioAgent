...
test('actual settings view decrypts only for an open panel and caches the public generation', () => {
  let decrypts = 0
  const context = vm.createContext({resolveSecretConfiguration, developmentEnv: {}, frontendUsage: {snapshot: () => ({})}, wakeWord: null, settingsWindow: null, capabilityEditorCache: null, settingsGeneration: 0, currentSettings: {}, process: {env: {}},
...
test('actual main refreshes a hand-edited registry while the panel is open and on backend launch', async t => {
  ...
  const context = vm.createContext({resolveSecretConfiguration, developmentEnv: {}, frontendUsage: {snapshot: () => ({})}, wakeWord: null, settingsWindow: {show() {}, focus() {}}, refreshManagedWorkspaceCapabilities: () => Promise.resolve(), sendToSettings: () => {}, capabilityEditorCache: null, settingsGeneration: 0,
