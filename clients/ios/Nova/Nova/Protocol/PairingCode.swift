...
struct PairingCode: Identifiable {
    ...
    static func parse(_ text: String, now: Date = Date()) throws -> PairingCode {
        ...
        guard text.utf8.count <= 4096,
              ...
              (payload.expires_at?.isFinite ?? true) else { throw WireError("这不是有效的 Nova 配对二维码。") }
        let expires = payload.expires_at.map { Date(timeIntervalSince1970: $0 / 1000) }
        guard expires.map({ $0 > now }) ?? true else { throw WireError("二维码已过期，请在 Mac 上重新生成。") }
