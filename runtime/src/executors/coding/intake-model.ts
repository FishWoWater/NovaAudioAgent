...
3. project: copy exactly one roster name verbatim, or null for the active project; for create, the new name taken from the utterance. session is a choice, not a description of the current session: {mode:'new'} when the user asks for a fresh thread; {mode:'named',title:<exact roster title>} only when the user explicitly names a session; otherwise {mode:'latest'}. Continue in the selected project without naming a session means {mode:'latest'}, which has no title field. For an explicitly named session that is absent or ambiguous, ask; never substitute latest. A uniquely user-named session can identify its project.
