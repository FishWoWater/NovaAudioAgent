...
export const assessSchema = intakeBindingSchema.extend({
  ...
  session_title: z.string().trim().min(1).max(80).nullable().default(null),
...
3. project: copy exactly one roster name verbatim, or null for the active project; for create, the new name taken from the utterance. session: 'new' only when the user asks for a fresh thread/session ("重新开一个", "新开一个会话"), otherwise 'latest'. If the user names a specific session, copy its exact title from that project's sessions into session_title; never substitute the latest session. If ambiguous or absent, ask. Without an explicitly user-named session use session_title null, even when active_project or roster lists a latest session title. A unique session title may identify its project even if the user did not also say the project name.
4. project_evidence: whenever project differs from active_project, copy the exact span of the user's utterance that names that project (for "改博客的暗色模式" with roster name 博客 → "博客"); the host rejects a span that does not contain the roster name (or is not contained in it), so a translation like "博客" for blog cannot serve as evidence — when the user's words do not contain any roster name, kind unclear and ask which project. When the previous host question was "是在 X 里做吗？", select project X only if the latest answer affirms that project, and quote the exact affirmative span from that answer as project_evidence. A refusal, uncertainty or unrelated amendment is kind unclear, not an affirmation. This host-question case is the only exception to quoting the roster name. null when project is the active one. For create, the goal slot covers only coding work inside the new project: a bare create request leaves goal missing.
