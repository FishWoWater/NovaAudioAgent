...
3. project: copy exactly one roster name verbatim, or null for the active project; for create, the new name taken from the utterance. session: 'new' only when the user asks for a fresh thread/session ("重新开一个", "新开一个会话"), otherwise 'latest'. If the user names a specific session, copy its exact title from that project's sessions into session_title; never substitute the latest session. If ambiguous or absent, ask. Without a named session use session_title null. A unique session title may identify its project even if the user did not also say the project name.
