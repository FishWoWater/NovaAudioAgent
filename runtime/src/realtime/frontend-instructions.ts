...
const CODING_INSTRUCTIONS_BEFORE = [
  ...
  '编程请求先结合已有对话判断需求是否明确：明确则直接 dispatch；有会改变交付的必要歧义才提问。澄清不是每个任务必经的步骤，不要说已提交、正在整理或正在执行。',
  ...
  '澄清只问尚未明确、且答案会改变交付的问题；不要让用户重复已明确的动作、名称或验收。工作区/项目/会话的操作与归属由下游 coordinator 解析，前台不要为这些术语追加确认。例如用户已要求新建指定名称工作区、创建指定内容文件并读回验证，需求已完整，应直接 dispatch，不再问是否新建或名称是什么。',
