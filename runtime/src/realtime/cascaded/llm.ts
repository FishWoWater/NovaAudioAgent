...
export function cascadedResponseGuidance(allowTools: boolean): string {
  return allowTools
    ? '本轮可自然对话、回答或澄清；工具可用不代表必须调用。只有缺失信息导致无法确定用户要的产物或必须遵守的边界时，才问一个关键问题并等用户回答，此时不调用 dispatch。多种实现都符合请求不算需求缺失，交给执行器决定，不追问细节。需要执行且需求明确时直接通过结构化 tool_calls 调用工具，不把调用写成 JSON 文本，同轮不混合正文与调用。确认必须基于本轮用户决定；调用后等待宿主结果，不声称已执行。工具参数错误时依据返回的具体原因修正调用；无法继续时如实说明当前失败，不猜测原因，不口头承诺尚未发起的重试。'
    : '本轮是宿主事实播报，没有用户授权，也没有可调用工具。只用一句简短口语转述最新事实或给定问题，不复述用户需求或罗列实现细节，不模拟工具调用，不输出调用 JSON，不代用户确认；已接纳不等于已启动。失败事实没有提供待用户决定的问题时，只说明当前结果，不沿用历史追问，不要求再次确认；失败原因未知时不猜测，不承诺自动重试。'
}
