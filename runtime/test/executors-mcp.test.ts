...
test('actual desktop entry awaits discovery and owns cleanup when final exact frontend budget rejects construction', async () => {
  ...
  const replacements = {
    [new URL('../src/config/config.js', import.meta.url).href]: `import {loadSettings as load} from ${JSON.stringify(configUrl)}; export {requireBlockingCredentials, requireIntegratedRealtime, withoutUncredentialedModules} from ${JSON.stringify(configUrl)}; export function loadSettings() {return {...load({MODEL_API_KEY:'fixture', DASHSCOPE_API_KEY:'fixture'}),executors:[]}}`,
