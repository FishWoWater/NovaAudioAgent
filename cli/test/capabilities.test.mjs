...
test('doctor uses the normalized persisted registry path before environment and default fallbacks', async () => {
  ...
  try {
    ...
    const selected = await inspect({CAPABILITIES_CONFIG: inherited})
    ...
    for (const capabilitiesConfigPath of ['', null, 12, 'bad\u0000path', 'x'.repeat(32769)]) {
      ...
      assert.equal((await inspect({CAPABILITIES_CONFIG: inherited})).capabilities.modules.knowledge.enabled, true)
    ...
    const missing = await inspect({CAPABILITIES_CONFIG: inherited})
