...
test('public version and target matrix are stable', () => {
  assert.equal(PRODUCT_VERSION, '0.2.3')
  assert.equal(resolveTarget('darwin', 'arm64').artifact, 'nova-audio-agent-0.2.3-macos-arm64-app.zip')
  ...
  assert.equal(resolveTarget('linux', 'x64').artifact, 'nova-audio-agent-0.2.3-linux-x64.AppImage')
...
test('release and settings paths use the documented local roots', () => {
  ...
  assert.equal(
    ...
    join('/tmp/home', '.nova-audio-agent', 'cli', 'releases', '0.2.3', 'win32-x64'),
  )
  assert.equal(
  ...
  assert.equal(releaseBaseUrl(), 'https://github.com/deepnovacore/NovaAudioAgent/releases/download/v0.2.3')
})
