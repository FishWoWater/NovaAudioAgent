...
const ARTIFACT = 'nova-audio-agent-0.2.3-windows-x64-portable.zip'
...
test('checksum failure leaves no runnable installation', async () => {
  ...
  const root = join(home, '.nova-audio-agent/cli/releases/0.2.3/win32-x64')
...
test('an interrupted download leaves no partial executable', async () => {
  ...
  const executable = join(home, '.nova-audio-agent/cli/releases/0.2.3/win32-x64/Nova Audio Agent Desktop.exe')
